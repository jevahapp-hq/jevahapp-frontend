...
/**
 * Stable empty reference. Zustand reads through `useSyncExternalStore`, whose
 * snapshot must be referentially stable — returning a fresh `[]` for every
 * un-loaded contentId made React re-render forever ("Maximum update depth
 * exceeded") anywhere a comment list or count was mounted.
 */
const NO_COMMENTS: any[] = [];

export const useComments = (contentId: string) =>
  useInteractionStore((state) => state.comments[contentId] || NO_COMMENTS);
