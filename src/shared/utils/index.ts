...
export const getContentKey = (item: MediaItem): string => {
  return `${item.contentType}-${item._id || Math.random().toString(36).substring(2)}`;
};
...
export const getUserDisplayNameFromContent = (item: MediaItem): string => {
  if (item.authorInfo?.firstName) {
    return `${item.authorInfo.firstName} ${item.authorInfo.lastName || ''}`.trim();
  }
...
export const getUserAvatarFromContent = (item: MediaItem): any => {
  const avatar = item.authorInfo?.avatar || item.author?.avatar || item.speakerAvatar;
...
export const getDisplayName = (speaker?: string, uploadedBy?: string): string => {
...
export const transformApiResponseToMediaItem = (item: any): MediaItem => {
  ...
  const thumbnailUrl = isValidUri(rawThumbnailUrl)
    ? String(rawThumbnailUrl).trim()
    : videoUrl; // Use video as thumbnail if no thumbnail
  ...
  return {
    ...
    speaker: item.authorInfo?.firstName || item.author?.firstName
...
export const isValidContentType = (type: string): boolean => {
  const validTypes = [
    'video', 'videos', 'audio', 'music', 'sermon', 
    'image', 'ebook', 'books', 'live', 'teachings', 'e-books'
  ];
...
export const isDownloadableContent = (item: MediaItem): boolean => {
  const downloadableTypes = ['video', 'videos', 'audio', 'music', 'image', 'ebook', 'books'];
