  ...
  async sendAuthRequest(token: string, userInfo: UserInfo) {
    ...
    console.log('📤 Request payload:', {
      ...
      userInfo: {
        ...
        avatar: userInfo.avatar ? `${userInfo.avatar.substring(0, 50)}...` : 'null'
      }
    });
    ...
    try {
      ...
      if (!response.ok) {
        console.error('❌ Backend returned error:', {
          status: response.status,
          result: result,
          errorDetails: result.error || result.details || 'No additional error details'
        });
        
        // Check if it's a Clerk signing key error
        if (result.error && result.error.includes('Failed to retrieve signing key from Clerk')) {
          console.log('🔧 Clerk signing key error detected. This is a backend configuration issue.');
          ...
          console.log('   3. Ensure Clerk environment variables match your app');
          
          throw new Error('Clerk configuration error: Please check your backend Clerk setup');
        }
        ...
        if (response.status === 500) {
          ...
          throw new Error(`Bad request: ${result.message || 'Invalid request data'}`);
