  ...
  return useRef(
    PanResponder.create({
      ...
      onPanResponderGrant: (evt) => {
        setIsSeeking(true);
        // Measure once on grant
        measureProgressBar();
        // Calculate initial progress immediately
        const touchX = evt.nativeEvent.locationX;
        const progress = Math.max(0, Math.min(1, touchX / progressBarWidthRef.current));
        setSeekProgress(progress);
      },
      onPanResponderMove: (evt, gestureState) => {
        // Use pageX for more accurate positioning across the entire bar
        const touchX = evt.nativeEvent.pageX - progressBarPageXRef.current;
        const progress = Math.max(0, Math.min(1, touchX / progressBarWidthRef.current));
        setSeekProgress(progress);
      },
