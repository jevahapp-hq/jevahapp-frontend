...
export default function ExploreSearch() {
  ...
  const testDownloadFromSearch = async () => {
    if (displayResults.length > 0) {
      ...
      const contentType =
        testItem.contentType === "music"
          ? "audio"
          : testItem.contentType === "videos"
          ? "video"
          : testItem.contentType === "books"
          ? "ebook"
          : testItem.contentType === "live"
          ? "live"
          : "video";
  ...
  const navigateToReels = (item: MediaItem, index: number) => {
    ...
    router.push({
      ...
      params: {
        ...
        speakerAvatar:
          typeof item.speakerAvatar === "string"
            ? item.speakerAvatar
            : require("../../assets/images/Avatar-1.png").toString(),
          ...
          <View className="absolute top-2 right-2 bg-white shadow-md rounded-lg p-2 z-50 w-32">
            ...
            <TouchableOpacity
              ...
              onPress={async () => {
                try {
                  console.log(
                    "🔍 Download button pressed for item:",
                  ...
                  console.log(
                    "📦 Converted downloadable item:",
                    JSON.stringify(downloadableItem, null, 2)
                  );
                  const result = await handleDownload(downloadableItem);
            ...
            >
              <Text className="text-[#1D2939] font-rubik text-sm">
                {checkIfDownloaded(item._id || item.fileUrl)
                  ? "Downloaded"
                  : "Download"}
              </Text>
              <Ionicons
        ...
        <View className="mb-4">
          <Text className="text-gray-700 text-base font-rubik-semibold mb-2">
            {hasSearched
              ...
              : `All Content (${displayResults.length})`}
          </Text>
        </View>
