...
export default function PrayerWallScreen() {
  ...
  const handleLike = async (prayer: PrayerRequestType) => {
    const prayerId = prayer._id || (prayer as any).id;
    if (!prayerId) return;
    await likePrayer(
      prayerId,
  ...
  const renderPrayerCard = (prayer: PrayerRequestType, index: number) => {
    // Handle both _id (MongoDB) and id fields from backend
    const prayerId = prayer._id || (prayer as any).id;
    ...
    return (
      <View key={prayerId || `prayer-${index}`} style={styles.cardWrapper}>
        <TouchableOpacity
      ...
      >
        ...
        {loading && (!prayers || prayers.length === 0) ? (
          ...
          <FlatList
            ...
            data={displayPrayers}
            keyExtractor={(item, index) => item._id || (item as any).id || `prayer-${index}`}
