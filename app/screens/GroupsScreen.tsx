          ...
          <>
            {loading && (!groups || groups.length === 0) ? (
              <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', paddingVertical: 60 }}>
                ...
                <Text style={{ marginTop: 16, fontSize: 16, color: '#6B7280', fontFamily: 'PlusJakartaSans-Regular' }}>Loading groups...</Text>
              </View>
              ...
              error.code === "HTTP_ERROR" && error.error?.includes("404") ? (
                ...
                <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', paddingVertical: 60, paddingHorizontal: 40 }}>
                  ...
                  <Text style={{ fontSize: 20, fontWeight: 'bold', color: '#1F2937', marginTop: 16, fontFamily: 'PlusJakartaSans-Bold' }}>Error loading groups</Text>
                  <Text style={{ fontSize: 14, color: '#6B7280', textAlign: 'center', marginTop: 8, fontFamily: 'PlusJakartaSans-Regular' }}>{error.error}</Text>
                  <TouchableOpacity
                    style={{ backgroundColor: '#1C8E79', paddingHorizontal: 24, paddingVertical: 12, borderRadius: 8, marginTop: 16 }}
                    onPress={refresh}
                    activeOpacity={0.7}
                  >
                    <Text style={{ color: 'white', fontSize: 16, fontWeight: '600', fontFamily: 'PlusJakartaSans-SemiBold' }}>Retry</Text>
                  </TouchableOpacity>
                </View>
              )
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <View style={{ backgroundColor: '#E9F1EF', borderRadius: 16, padding: 16, marginHorizontal: 20, marginBottom: 16, flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    {/* Info */}
                    <View style={{ flex: 1 }}>
                      <Text style={{ fontSize: 16, fontWeight: 'bold', color: '#000', marginBottom: 4, fontFamily: 'PlusJakartaSans-Bold' }}>{item.name}</Text>
                      <Text style={{ fontSize: 14, color: '#666', lineHeight: 20, fontFamily: 'PlusJakartaSans-Regular' }} numberOfLines={2}>{item.description || 'Group'}</Text>
                      <Text style={{ marginTop: 6, color: '#6B7280', fontSize: 12, fontFamily: 'PlusJakartaSans-Regular' }}>{`${(item.membersCount || 0).toLocaleString()} Members`}</Text>
                    </View>
                  ...
                  {menuGroup ? (
                    <>
                      <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, paddingBottom: 8 }}>
                        ...
                        <Text style={{ flex: 1, fontFamily: 'PlusJakartaSans-Bold', color: '#0F172A', fontSize: 12 }} numberOfLines={2}>
                          {menuGroup.name}
                        </Text>
                      </View>
                      ...
                      <TouchableOpacity activeOpacity={0.8} style={{ paddingVertical: 10, paddingHorizontal: 16 }} onPress={() => { setMenuVisible(false); router.push({ pathname: '/screens/GroupChatScreen', params: { groupId: menuGroup._id, groupTitle: menuGroup.name, groupDescription: menuGroup.description, groupMembers: String(menuGroup.membersCount || 0) } }); }}>
                        <Text style={{ fontFamily: 'PlusJakartaSans-Bold', color: '#0F172A', fontSize: 12 }}>Group Details</Text>
                      </TouchableOpacity>
                      <TouchableOpacity
                        activeOpacity={0.8}
                      ...
                      >
                        <Text style={{ fontFamily: 'PlusJakartaSans-Bold', fontSize: 12, color: (menuGroup.userRole || menuGroup.role || "").toLowerCase() === 'admin' ? '#0F172A' : '#98A2B3' }}>Edit group Details</Text>
                      </TouchableOpacity>
                      <TouchableOpacity activeOpacity={0.8} style={{ paddingVertical: 10, paddingHorizontal: 16 }} onPress={() => { setMenuVisible(false); setShowDelete(true); }}>
                        <Text style={{ fontFamily: 'PlusJakartaSans-Bold', color: '#DC2626', fontSize: 12 }}>Delete group</Text>
                      </TouchableOpacity>
                      <TouchableOpacity activeOpacity={0.8} style={{ paddingVertical: 10, paddingHorizontal: 16 }} onPress={() => { setMenuVisible(false); setShowFlag(true); }}>
                        <Text style={{ fontFamily: 'PlusJakartaSans-Bold', color: '#DC2626', fontSize: 12 }}>Flag group</Text>
                      </TouchableOpacity>
                    </>
                  ) : null}
                </View>
