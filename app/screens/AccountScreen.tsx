...
import { Alert, Dimensions, Image, Modal, Pressable, ScrollView, Text, TouchableOpacity, TouchableWithoutFeedback, View } from "react-native";
...
export default function AccountScreen() {
  ...
  return (
        ...
        <View className="px-4 py-8">
          ...
          <View className="flex-row items-center mb-4">
            ...
            <View style={{ flex: 0.98 }} className="items-end">
              ...
              >
                <Text className="font-medium" style={{ color: '#0A332D' }}>Edit</Text>
                <Ionicons name="pencil-outline" size={16} style={{ marginLeft: 6 }} color="#0A332D" />
              </TouchableOpacity>
            </View>
          </View>
          ...
          <View className="bg-gray-200 rounded-xl px-2 py-1 flex-row items-center justify-between w-10/12 self-center">
            {contentTabs.map((tab, index) => (
              <TouchableOpacity
                ...
                activeOpacity={0.7}
                style={selectedContentTab === index ? { backgroundColor: '#0A332D' } : {}}
              >
                <Ionicons 
          ...
          {selectedContentTab !== 3 ? (
            <View className="flex-row justify-between">
              {tabImages[selectedContentTab].map((imgSrc, index) => (
                <View key={`tile-${selectedContentTab}-${index}`} className="flex-1 mx-1">
            ...
            <View>
              {[
                { icon: 'albums-outline', label: 'Posts', value: '1200', sub: 'Total published posts' },
                { icon: 'heart-outline', label: 'Likes', value: '16.8k', sub: 'Number of "Like" engagements on all posts' },
                { icon: 'radio-outline', label: 'Live sessions', value: '32', sub: 'Number of times you went Live' },
                { icon: 'chatbubble-ellipses-outline', label: 'Comments', value: '20k', sub: 'Number of "comments" on all posts' },
                { icon: 'document-text-outline', label: 'Drafts', value: '25', sub: 'Unpublished posts' },
                { icon: 'share-social-outline', label: 'Shares', value: '0', sub: 'Number of times people shared your contents' },
              ].map((row, idx) => (
                <View key={`row-${idx}`} className="flex-row items-center justify-between bg-white rounded-2xl px-4 py-4" style={{ shadowColor: '#000', shadowOpacity: 0.03, shadowRadius: 8, marginBottom: 16 }}>
                  <View className="flex-row items-center">
                  ...
                  <Text className="text-[#111827] font-semibold">{row.value}</Text>
                </View>
              ))}
            </View>
          )}
    ...
    <EditProfileSlideOver visible={isEditOpen} onClose={() => setIsEditOpen(false)} />
    </>
  );
}